...
Paging refers to getting successive batches of results. The Streaming API endpoints in a sense do this inherently. REST API endpoints will never return more than a specificied maximum number of results. When you request `search/tweets`, for example, by default you will get at most 20 tweets. You can increase that number to a maximum of 100. This is the page size. Twitter `provides <http://dev.twitter.com/rest/public/timelines>`_ a way to get successive pages, so it is possible to get more than 100 tweets with `search/tweets`, just not in a single request.
